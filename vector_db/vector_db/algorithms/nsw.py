...
class NSW:
    ...
    def insert(self, value: list[float]):
        node = Node(value)
        if not self.nodes:
            self.nodes.append(node)
            return
        all_neighbours = [self._find_closest(value)]
        i = 0
    ...
    def _distance_compare(self, node1: Node, node2: Node, /):
        return sum((a - b) ** 2 for a, b in zip(node1.value, node2.value))
    ...
    def _compare_a_to_c_and_a_to_b_to_c(self, a: Node, b: Node, c: Node) -> bool:
        ...
        b_to_c = self._distance_compare(b, c) ** 0.5
        return a_to_c <= a_to_b + b_to_c
